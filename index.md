...
Данный сайт содержит документацию к разработанным мной пакетам функций на языке R, каждый из пакетов представляет из себя набор функций для упрощённой работы с API различных сервисов: Facebook, Вконтакте, Яндекс Директ, Яндекс Метрика, MyR=target, Mixpanel, GitHub.
...
Все пакеты распространяются совершенно свободно из моего репозитория на GitHub, но для их установки изначально вам потребуется установить пакет `devtools`, для его установки запустите RStudio и запустите коменду `install.packages("devtools")`.
